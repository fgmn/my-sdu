 ...
 * @LastEditors: Wangzhe
 * @LastEditTime: 2020-11-06 19:14:44
 * @FilePath: \lab3\src\pipe.c
 */
/*
...
void pipe_cycle()
{
    ...
    // if (!FLAG) {
        pipe_stage_wb();
        pipe_stage_mem();   // 1 cylce -> 51 cycles
    // } 
    // if (is_substitude) {
    //     locks = 50;
    // }
...
void pipe_stage_decode()
{
    ...
    uint32_t opcode = (op->instruction >> 26) & 0x3F;       //提取6位，指令编码
    uint32_t rs = (op->instruction >> 21) & 0x1F;           //提取5位
    uint32_t rt = (op->instruction >> 16) & 0x1F;           //提取5位
    uint32_t rd = (op->instruction >> 11) & 0x1F;           //提取5位
    uint32_t shamt = (op->instruction >> 6) & 0x1F;         //偏移量
    uint32_t funct1 = (op->instruction >> 0) & 0x1F;        //后5位
    uint32_t funct2 = (op->instruction >> 0) & 0x3F;        //后6位
    uint32_t imm16 = (op->instruction >> 0) &0xFFFF;       //后16位
...
void pipe_stage_fetch()
{
    ...
    op->reg_src1 = op->reg_src2 = op->reg_dst = -1;     //初始化

    // op->instruction = mem_read_32(pipe.PC);
    op->instruction = cache_read_32(pipe.PC);
    op->pc = pipe.PC;
    pipe.decode_op = op;
