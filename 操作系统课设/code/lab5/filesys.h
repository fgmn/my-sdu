...
class FileSystem {
  public:
	...
	bool FindName(char *name, char *fileName);
	bool CreateTest(char *name, int initialSize);
	OpenFile *OpenTest(char *name);
	bool RemoveTest(char *name, int cascade); // Delete a file (UNIX unlink)
	bool cd(char *name);
	void pwd();
	void ListTest(); // List all the files in the file system
	bool cat(char *name);
	void treeTest();
	void tree(int sector, char *prefix);
