...
#define FilePathMaxLen 40 // 为了简单起见，我们假设绝对文件路径 <= 40字符
#define DirType 0   // 目录文件 & 普通文件
#define FileType 1
...
class DirectoryEntry {
  public:
    ...
    // 文件头（i-Node）所在扇区号，Nachos文件头恰好占用一个扇区！
    char name[FileNameMaxLen + 1];	// Text name for file, with +1 for 
					// the trailing '\0'

    // 目录项体积增加
    int type;
    char path[FilePathMaxLen + 1];  // may be redundant now
};
...
class CurDir 
{ // 维护当前路径，因为Nachos没有内存概念，当前路径从硬盘中获取
  public:
    CurDir() {};
