...
// 从外部引入函数
extern void CopyTest(char *unixFile, char *nachosFile);
extern void AppendTest(char *unixFile, char *nachosFile, int half);
extern void NAppendTest(char *nachosFileFrom, char *nachosFileTo);
...
//    ThreadTest();
	...
	// }
		...
		{ // copy from UNIX to Nachos
			ASSERT(argc > 2);
			CopyTest(*(argv + 1), *(argv + 2));
			argCount = 3;
		}
		else if (!strcmp(*argv, "-ap"))
		{ // append from UNIX to Nachos
			ASSERT(argc > 2);
			AppendTest(*(argv + 1), *(argv + 2), 0);
			argCount = 3;
		}
		else if (!strcmp(*argv, "-hap"))
		{
			// cut half and append from UNIX to Nachos
			ASSERT(argc > 2);
			AppendTest(*(argv + 1), *(argv + 2), 1);
			argCount = 3;
		}
		else if (!strcmp(*argv, "-nap"))
		{ // append from Nachos to Nachos
			ASSERT(argc > 2);
			NAppendTest(*(argv + 1), *(argv + 2));
			argCount = 3;
		}
		...
		{ // copy from UNIX to Nachos
			ASSERT(argc > 1);
			fileSystem->CreateTest(*(argv + 1), -1);
			argCount = 2;
		}
		else if (!strcmp(*argv, "-cptest"))
		{ // copy from UNIX to Nachos
			ASSERT(argc > 2);
			CopyTest(*(argv + 1), *(argv + 2));
			argCount = 3;
		...
		{ // remove Nachos file
			ASSERT(argc > 1);
			fileSystem->RemoveTest(*(argv + 1), 0);
			argCount = 2;
		}
		...
		else if (!strcmp(*argv, "-rmdir-r"))
		{ // remove Nachos dir
			ASSERT(argc > 1);
			fileSystem->RemoveTest(*(argv + 1), 1);
			argCount = 2;
		}
		else if (!strcmp(*argv, "-aptest"))
		{ // append from UNIX to Nachos
			ASSERT(argc > 2);
			AppendTest(*(argv + 1), *(argv + 2), 0);
			argCount = 3;
		}
		else if (!strcmp(*argv, "-haptest"))
		{
			// cut half and append from UNIX to Nachos
			ASSERT(argc > 2);
			AppendTest(*(argv + 1), *(argv + 2), 1);
