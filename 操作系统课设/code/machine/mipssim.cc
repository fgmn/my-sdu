...
Machine::Run()	// to comprehend
{
    ...
    for (;;) {	// 循环读取一条指令并执行，功能类似于CU
        OneInstruction(instr);
		interrupt->OneTick();	// 这里开中断，允许并发
...
{	
    ...
    // Fetch instruction 
	// 读取指令，PC寄存器中存储的是虚拟地址，因此首先需要地址转换
	...
	break;
      ...
      case OP_BEQ:	// 条件跳转指令，操纵NextPCReg寄存器
    ...
    registers[NextPCReg] = pcAfter;		// 跳转到此处
}
