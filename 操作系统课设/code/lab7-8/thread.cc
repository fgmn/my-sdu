...
{ // 栈分配
    ...
    stackTop = stack + StackSize - 4; // -4 to be on the safe side!
                                      ...
                                      // ThreadRoot.
                                      // 在此处将SWITCH()返回地址设置为ThreadBoot地址
...
{ // 当前进程为Joiner，SpaceId对应Joinee
    ...
    const char *ans = interminatedList ? "yes" : "no";
    DEBUG('x', "Is it in tList? %s\n", ans);
    // 首先判断是否在终止队列中end
    ...
    {                                      // 将Joinee加入等待队列
        ...
        DEBUG('x', "thread %s join sleep\n", currentThread->getName());
        // Joiner进入睡眠，等待Joinee执行Finish将其唤醒
        currentThread->Sleep();
        DEBUG('x', "thread %s join wake up\n", currentThread->getName());
    ...
    //  此时不需要将Joinee加入等待队列，获取Joinee进程退出码
